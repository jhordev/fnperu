...
class CursosModel extends Model
{
    ...
    {
        return $this -> where(['curso_estado' => 1]) -> orderBy(['curso_publico' => 'desc', 'curso_orden' => 'asc', 'curso_creacion' => 'desc']) -> findAll();
    }
    ...
    {
        return $this -> where(['curso_estado' => 1, 'curso_publico' => 1]) -> orderBy(['curso_orden' => 'asc', 'curso_creacion' => 'desc']) -> findAll();
    }
    ...
    public function getLastCursosPublicadosConPrecio(string $fechaMaxFin)
    {
        ...
        WHERE lanzamiento.lanzamiento_eliminado = 0 AND cursos.curso_estado = 1 AND lanzamiento.lanzamiento_fin >= :lanzamiento_fin
        ORDER BY cursos.curso_orden ASC, cursos.curso_id, lanzamiento.lanzamiento_estado DESC, lanzamiento.lanzamiento_inicio DESC');
        $update -> bindValue(':lanzamiento_fin', $fechaMaxFin);
    ...
    public function getCursoPublicadoConPrecioByIdCurso(int $idCurso, string $fechaMaxFin)
    {
        $update = $this -> query('SELECT * FROM cursos
        INNER JOIN lanzamiento ON lanzamiento.lanzamiento_curso = cursos.curso_id
        WHERE lanzamiento.lanzamiento_eliminado = 0 AND cursos.curso_estado = 1 AND lanzamiento.lanzamiento_fin >= :lanzamiento_fin AND cursos.curso_id = :curso_id
        ORDER BY lanzamiento.lanzamiento_estado DESC, lanzamiento.lanzamiento_inicio DESC');
        $update -> bindValue(':lanzamiento_fin', $fechaMaxFin);
        $update -> bindValue(':curso_id', $idCurso);
        $result = $update -> execute();
